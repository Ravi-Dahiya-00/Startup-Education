...
import { 
  Search, MapPin, DollarSign, Clock, Briefcase, Bookmark, 
  Share2, Heart, Users, Navigation, Filter, X, TrendingUp,
  Building2, Calendar, Award, ChevronRight
} from 'lucide-react';
...
const USER_TYPES = ['Professional', 'Fresher', 'Experienced'];
const DATE_RANGES = [
...
const DOMAINS = ['Engineering', 'Design', 'Marketing', 'Product', 'Sales', 'Data Science', 'Content', 'Finance'];
...
const Jobs = () => {
  ...
  const { detectLocation, location: userLocation, loading: locLoading } = useUserLocation();
  const {
    query: locationQuery,
  ...
  const fetchJobs = async () => {
    try {
      ...
      const enrichedData = data.map(item => ({
        ...item,
        workType: item.workType || WORK_TYPES[Math.floor(Math.random() * WORK_TYPES.length)],
        workingDays: item.workingDays || WORKING_DAYS[Math.floor(Math.random() * WORKING_DAYS.length)],
        userType: item.userType || USER_TYPES[Math.floor(Math.random() * USER_TYPES.length)],
        postedAt: item.postedAt || new Date(Date.now() - Math.floor(Math.random() * 10 * 24 * 60 * 60 * 1000)).toISOString(), // Random date in last 10 days
        domain: item.domain || DOMAINS[Math.floor(Math.random() * DOMAINS.length)]
      }));
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    if (filters.workType.length > 0) {
      const match = filters.workType.some(type => 
    ...
    if (filters.workingDays.length > 0) {
      const match = filters.workingDays.some(days => 
        job.tags.some(tag => tag.toLowerCase().includes(days.toLowerCase())) ||
        (job.workingDays && job.workingDays.toLowerCase().includes(days.toLowerCase()))
      );
      if (!match) return false;
    }
    ...
    if (filters.userType.length > 0) {
      const match = filters.userType.some(type => 
        job.tags.some(tag => tag.toLowerCase() === type.toLowerCase()) ||
        (job.userType && job.userType.toLowerCase() === type.toLowerCase())
      );
      if (!match) return false;
    }
    ...
    if (filters.domain.length > 0) {
      const match = filters.domain.some(domain => 
        job.tags.some(tag => tag.toLowerCase() === domain.toLowerCase()) ||
        (job.domain && job.domain.toLowerCase() === domain.toLowerCase())
      );
      if (!match) return false;
    }
    ...
    if (filters.datePosted) {
      const postedDate = new Date(job.postedAt || Date.now());
      const now = new Date();
  ...
  const featuredCompanies = [
    { name: 'Google', logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/Google_%22G%22_logo.svg/768px-Google_%22G%22_logo.svg.png', count: 12 },
    { name: 'Microsoft', logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/44/Microsoft_logo.svg/2048px-Microsoft_logo.svg.png', count: 8 },
    { name: 'Amazon', logo: 'https://upload.wikimedia.org/wikipedia/commons/a/a9/Amazon_logo.svg', count: 15 }
  ];
  ...
  return (
    <div className={`jobs-page ${theme}-theme`}>
      {/* Premium Header */}
      ...
      <div className="jobs-layout">
        ...
        <aside className="filters-panel">
          <div className="filters-header">
            ...
            {(filters.type.length > 0 || filters.location.length > 0 || filters.skills.length > 0 || filters.workType.length > 0 || filters.workingDays.length > 0 || filters.datePosted || filters.userType.length > 0 || filters.domain.length > 0) && (
              <button 
          ...
          <div className="filter-group">
            <div className="filter-label-row">
              <label className="filter-label">Location ({filters.location.length} selected)</label>
              ...
              {locationSuggestions.length > 0 ? (
                locationSuggestions.map((location, idx) => {
                  ...
                  return (
                    ...
                    >
                      <MapPin size={12} />
                      <span className="location-name">{location.city ? `${location.city}, ${location.state}` : cityName}</span>
          ...
          {/* Skills Filter */}
          <div className="filter-group">
            <label className="filter-label">Skills ({filters.skills.length} selected)</label>
            <div className="skills-search-box">
              ...
              />
              {skillsSearch && (
                <button onClick={() => setSkillsSearch('')} className="clear-search">
                  <X size={14} />
                </button>
              )}
            </div>
            <div className="skills-grid">
              {(skillsSearch ? searchSkills(skillsSearch).slice(0, 30) : POPULAR_SKILLS).map(skill => (
                <button
                  key={skill}
              ...
              {filteredJobs.map((job, index) => (
                ...
                >
                    ...
                    <div className="company-info">
                      ...
                      <div className="company-details">
                        <h3 
                          ...
                          onClick={() => router.push(`/jobs/${job._id}`, { state: { job } })}
                  ...
                  <div className="card-footer">
                    <div className="footer-left">
                      <div className="badge-group">
                        <span className="badge">{job.userType}</span>
                        <span className="badge">{job.domain}</span>
                      </div>
                  ...
                  <div className="card-bottom-meta">
                    <span className="posted">{getPostedString(job.postedAt)}</span>
      ...
      <style>{`
        ...
        .apply-btn {
          ...
          text-decoration: none; /* conversion to <a> tag might need this */
        }
